...
$if(pemail)$ \texttt{\href{mailto:$pemail$}{$pemail$}} $endif$
$if(pphone)$ $pphone$ $endif$
$if(pweb)$ \texttt{\href{http://$pweb$}{$pweb$}} $endif$
\columnbreak % Force break to the next column
% Right column (Professional contact info)
$if(waddress)$ $waddress$ $endif$
$if(wemail)$ \texttt{\href{mailto:$wemail$}{$wemail$}} $endif$
$if(wphone)$ $wphone$ $endif$
$if(wweb)$ \texttt{\href{http://$wweb$}{$wweb$}} $endif$
\end{multicols}
